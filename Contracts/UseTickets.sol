...
//------------------------------------------------------   Functions  --------------------------------------------------------
        ...
        returns(string memory timelocked)
    {
        ...
        if (_CE_Tickets[_id].Aty == true) {
            _lockUnlock = Both;
            payable(_CE_Tickets[_id].Receiver).transfer(_amount.div(2));
        } else
            _lockUnlock = Locked;

        UpdateStatsCE(_id, _lockUnlock, _amount, _amount.div(2), NONE, 1);
        ...
        returns(string memory locked)
    {
        ...
        UpdateStatsCE(_id, Unlocked, _amount, _amount.div(2), Death, 2);
