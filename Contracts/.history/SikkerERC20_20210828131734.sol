...
contract SikkerToken is Owner {
    ...
    function transferFrom(address from, address to, uint256 value) public returns(bool) {
        require(value <= Tokens[from].Balance, "Requested address have not enough funds");
        require(value <= _allowed[from][msg.sender], "Allowance is not high enough");
        require(to != address(0), "Invalid address");

        Tokens[from].Balance = Tokens[from].Balance.sub(value);
        Tokens[to].Balance = Tokens[to].Balance.add(value);
    ...
    function transfer(address _receiver, address _sender, uint256 _amount) public OnlySender(_sender) {
        ...
        Tokens[_sender].Balance = Tokens[_sender].Balance.sub(_amount);
        Tokens[_receiver].Balance = Tokens[_receiver].Balance.add(_amount);
