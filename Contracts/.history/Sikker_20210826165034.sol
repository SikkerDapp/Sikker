...
contract Sikker is Owner {
    ...
    event CreatedTicket(uint256 _id, string _type, uint256 _amount, address _creator, address _receiver, bool _timelocked, uint8 _lossPercent, bytes32 _hash);
    event FilledCE(uint256 _id, uint256 amount, address _creator, address _payer, address _receiver);
    event ClosedTicket(uint256 _id, string _type, uint256 _amount, address _payer, bool _isTimeBlocked, uint8 _lostPercent);
    ...
    function _Total_CE_Tickets_Active() public view returns(uint256) {
        return(CE_Ticket_Number - CE_Inactive_Ticket_Number);
    }
    ...
    function _Total_TMM_Tickets_Active() public view returns(uint256) {
        return(TMM_Ticket_Number - TMM_Inactive_Ticket_Number);
    }

    function _TotalValueLocked() public view returns(uint256) {
        return(TotalValueEverLocked - TotalValueUnlocked);
    }
}